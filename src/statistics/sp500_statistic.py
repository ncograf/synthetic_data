...
class SP500Statistic(base_statistic.BaseStatistic):
    ...
    def set_statistics(self, data: pd.DataFrame):
        ...
        self._statistic = data.notna().sum(
            axis=0
        )  # compute number of valid datapoints in each symbol
        ...
        for key in data.columns[self.nan_after_start != 0]:
            self.dict_messy_series[key] = [
                self.nan_after_start[key],
                self.start_series[key],
                self.start_clean_series[key],
            ]
    ...
    def distribution_properties(self) -> Dict[str, any]:
        ...
        properties = {
            ...
            "Last clean start date symbol": self.start_clean_series.index[
                self.start_clean_series.argmax()
            ],
            "Last start date": self.start_series.max(),
            "Last start date symbol": self.start_series.index[
                self.start_series.argmax()
            ],
