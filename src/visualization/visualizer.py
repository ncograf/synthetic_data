...
def visualize_stylized_pair(
    ...
    plotter = visualize_pair(real_data=real_data, gen_data=gen_data, fact=fact)
    plotter.save()
...
def visualize_stylized_facts(loader: Literal["real", "g_1_1_norm", "g_3_3_student"]):
    ...
    if loader == "g_3_3_student":
        ...
        data_loader = gen_data_loader.GenDataLoader()
        stock_data = data_loader.get_timeseries(
            generator=garch,
            col_name="Adj Close",
        ...
        stock_data = real_loader.get_timeseries(
            col_name="Adj Close", data_path="data/raw_yahoo_data", update_all=False
        )
    else:
