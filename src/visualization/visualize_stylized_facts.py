...
from heavy_tails import heavy_tail_axes_setting, heavy_tail_plot_setting, heavy_tails
...
def visualize_stylized_facts(
    ...
    # HEAVY TAILS
    heavy_tail_density, heavy_tail_bins = heavy_tails(log_returns=log_returns)
    axes[0, 1].set(**heavy_tail_axes_setting)
    axes[0, 1].plot(heavy_tail_bins, heavy_tail_density, **heavy_tail_plot_setting)

    # VOLATILTIY CLUSTERING
    vol_clust_data = volatility_clustering(log_returns=log_returns, max_lag=1000)
    ...
    gain_data, loss_data = gain_loss_asymmetry(
        price=price_data, max_lag=1000, theta=0.1
    )
...
def visualize_averaged_stylized_facts(
    ...
    for log_returns in log_return_list:
        lin_unpred_list.append(
            linear_unpredictability(log_returns=log_returns, max_lag=1000)
        )
    ...
    # HEAVY TAILS
    ht_dens_list, ht_bin_list = [], []
    for log_returns in log_return_list:
        dens, bin = heavy_tails(log_returns=log_returns)
        ht_dens_list.append(dens)
        ht_bin_list.append(bin)
    heavy_tail_bins, heavy_tail_density = (
        np.mean(ht_bin_list, axis=0),
        np.mean(ht_dens_list, axis=0),
    )
    axes[0, 1].set(**heavy_tail_axes_setting)
    axes[0, 1].plot(heavy_tail_bins, heavy_tail_density, **heavy_tail_plot_setting)
    ...
    for log_returns in log_return_list:
        vol_clust_list.append(
            volatility_clustering(log_returns=log_returns, max_lag=1000)
        )
    ...
    gd_list, ld_list = [], []
    for price_data in price_data_list:
        gd, ld = gain_loss_asymmetry(price=price_data, max_lag=1000, theta=0.1)
        gd_list.append(gd)
        ld_list.append(ld)
