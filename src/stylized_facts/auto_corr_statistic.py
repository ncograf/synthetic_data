...
class AutoCorrStatistic(stylized_fact.StylizedFact):
    ...
    def _set_statistics_stride(self, data: pd.DataFrame | pd.Series | None = None):
        ...
        centered_shifted = np.lib.stride_tricks.sliding_window_view(
            padded, window_shape=centered.shape[0], axis=0, writeable=True
        )
        ...
        num_entries[:] -= np.flip(
            np.arange(1, self._max_lag + 1)
        )  # add the missing values because of the lag offest
    ...
    def _set_statistics_python_loop(self, data: pd.DataFrame | pd.Series | None = None):
        ...
        for lag in range(1, self._max_lag + 1):
            stat[lag - 1] = np.sum(base[lag:] * base[:-lag], axis=0) / (
                base.shape[0] - lag - num_nan
            )
