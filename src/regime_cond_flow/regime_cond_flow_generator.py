...
from regime_cond_flow import RegimeCondFlow
...
class RegimeCondFlowGenerator(base_generator.BaseGenerator):
    def fit(
        self,
        ...
        Args:
            ...
            config(Dict[str, Any]): configuration for training run:
                regime_cond_flow_config:
                    ...
                    regime_config (Dict[str, Any]): Regime configuration.
        ...
        dtype = config["regime_cond_flow_config"]["dtype"]
    ...
    def sample(
        ...
        model = RegimeCondFlow(**config["init_params"])
    ...
    def train_cond_flow(
        ...
        config: Dict[str, Any],
        accelerator: Accelerator,
    ) -> Tuple[RegimeCondFlow, float, float, float]:
        ...
        model_ff = RegimeCondFlow(**config["regime_cond_flow_config"])
        # log gradient information
        # run.watch(model, log="all")

        X_scaled, shift, scale = self.min_max_scaling(X)
        model_ff.set_normilizing(X_scaled)
        regime_data_list = model_ff.regime_detector.fit(X_scaled)

        for i in range(len(regime_data_list)):
            dataset = TensorDataset(X_scaled[regime_data_list[i]])
            loader = DataLoader(
                dataset=dataset, batch_size=batch_size, shuffle=True, pin_memory=True
            )

            optimizer = torch.optim.Adam(
                model_ff.regimes[i].parameters(), **config["optim_config"]
            )
            ...
            for epoch in range(epochs):
                ...
                for (batch,) in loader:
                    ...
                    _, log_prob_z, log_jac_det = model.forward(batch, regime=i)
                ...
                if epoch % n == n - 1:
                    print(
                        f"epoch/regime_{i}: {epoch + 1:>8d}/{epochs},\tlast loss {epoch_loss:>8.4f},\tlast learning rate {last_lr:>8.8f}"
                    )

            # free accelerator memory for next regime
            accelerator.free_memory()
