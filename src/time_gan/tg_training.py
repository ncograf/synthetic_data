...
def train_time_gan(
    ...
    embedder = TGRNNNetwork(T, hidden_dim=hidden_dim, num_layer=n_layer, in_dim=1, out_dim=1, bidirect=False)
    recovery = TGRNNNetwork(T, hidden_dim=hidden_dim, num_layer=n_layer, in_dim=1, out_dim=1, bidirect=False)
    # The supervisor is a hack to make the teacher forcing work
    supervisor = TGRNNNetwork(T, hidden_dim=hidden_dim, num_layer=n_layer, in_dim=1, out_dim=1, bidirect=False)
    generator = TGRNNNetwork(T, hidden_dim=hidden_dim, num_layer=n_layer, in_dim=z_dim, out_dim=1, bidirect=False)
    discriminator = TGRNNNetwork(T, hidden_dim=hidden_dim, num_layer=n_layer, in_dim=1, out_dim=1, bidirect=True)
    ...
    for epoch in range(epochs):
        ...
        for x, in data_loader:
            ...
            h = embedder(x)
            x_tilde = recovery(h)
            loss = torch.mean((x_tilde.flatten() - x.flatten())**2) # reconstructed x and real x
            loss.backward()
            opt_recon.step()
            #scd_recon.step()
        ...
        if epoch % 10 == 0:
            print((f'Reconstruction Epoch: {epoch:>10d}, last loss {loss.item():>15.4f},'
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            print((f'Supervised Epoch: {epoch:>10d}, last loss {loss.item():>15.4f},'
    ...
    for epoch in range(epochs):
        ...
        for _ in range(n_gen):
            ...
            for x, in data_loader:
                ...
                h_real = embedder(x)
                h_gen = generator(z)
                h_hat= supervisor(h_gen) # generator and supervisor are used to enable teacher forcing
                ...
                loss_supervisor = torch.mean((h_real[:,:-1] - h_hat_super[:,1:])**2) # supervisor loss
                loss_embedder = torch.mean((x.flatten() - x_hat_real.flatten())**2) # embedder loss
                loss_emb_sup = 10 * torch.sqrt(loss_embedder) + 0.1 * loss_supervisor

                # moment losses not mentioned in the paper but in the code
                loss_std = torch.mean(torch.abs(torch.std(x_hat, dim=0) - torch.std(x, dim=0)))
                loss_mu = torch.mean(torch.abs(torch.mean(x_hat, dim=0) - torch.mean(x, dim=0)))
                loss_moments = loss_mu + loss_std
                
                # generator losses
                loss_fake_gen = cross_entropy_loss(torch.ones_like(y_fake_gen), y_fake_gen)
                loss_fake = cross_entropy_loss(torch.ones_like(y_fake), y_fake)
                loss_generator = loss_fake + gamma * loss_fake_gen + 100 * torch.sqrt(loss_supervisor) + 100 * loss_moments
        ...
        for x, in data_loader:            
            ...
            y_fake = discriminator(h_hat)
            y_fake_gen = discriminator(h_gen)
            y_real = discriminator(h_real) # I think these outputs are shifted back by one as they are not fed throught the supervisor

            # discriminator loss
            loss_disc_fake_gen = cross_entropy_loss(torch.zeros_like(y_fake_gen), y_fake_gen)
        ...
        if epoch % 10 == 0:
            ...
            print((f'{"Last Embedding loss":<25} {loss_emb_sup.item():>10.4f},'
                f' aveage_loss {np.mean(losses["embed"]):>10.4f}'))
            print((f'{"Last Discriminator loss":<25} {loss_disc.item():>10.4f},'
                f' aveage_loss {np.mean(losses["discr"]):>10.4f}'))
            print((f'{"Last Generator loss":<25} {loss_generator.item():>10.4f},'
