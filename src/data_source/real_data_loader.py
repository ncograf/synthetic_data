...
class RealDataLoader:
    ...
    def _get_local_data(
        ...
        for symbol in symbols:
            ...
            try:
                ...
                error_dict[symbol] = (
                    f"Problem reading csv with pandas for symbol {symbol}:\n{str(ex)}"
                )
                continue
    ...
    def _get_yahoo_data(
        ...
        for i, symbol in enumerate(symbols):
            ...
            try:
                ...
                print(
                    f"Found {data.shape[0]} datapoints from {data.index[0].strftime('%m-%d-%Y')} to {data.index[-1].strftime('%m-%d-%Y')}.",
                    flush=True,
                )
    ...
    def _get_all_data(
        ...
        if not update_all:
            data_dict, error_dict = self._get_local_data(
                symbols=symbols, data_path=data_path, required_columns=required_columns
            )
    ...
    def get_timeseries(
        ...
        data_dict = self.get_all_data(
            data_path=data_path,
            required_columns=["Date", col_name],
            update_all=update_all,
        )
