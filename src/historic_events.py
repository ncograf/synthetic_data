...
class HistoricEventSet(base_outlier_set.BaseOutlierSet):
    ...
    def _dict_to_list(
            self,
            dic : Dict[pd.Timestamp, List[HistoricEvent]]
            ) -> List[Dict[str, any]]:
    ...
    def cmd_edit_event(self, date : pd.Timestamp, symbol : str | None = None):
        ...
        if date in self._events:
            try:
                ...
                click.echo(f"Error occurred during change event, previous state was restored. {e}")
                return
