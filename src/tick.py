...
class Tick:
    ...
    def __init__(self, date : pd.Timestamp, symbol : str, note : str = "", real : bool = True):
    ...
    def get_dict(self) -> Dict[str, pd.Timestamp | str | bool]:
        ...
        """
        return {'date' : self.date.strftime("%Y-%m-%d"), 'note' : self.note, 'real' : self.real}
    ...
    @staticmethod
    def unzip(ticks : Set[Tick], filter_unreal : bool = True) -> Tuple[List[pd.Timestamp], List[str]]:
