...
class TestLocalData:
    ...
    def test_local_data_basic(self):
        ...
        assert set(data_dict["ABBV"].columns) == set(
    ...
    def test_local_data_error(self):
        ...
        data_dict, error_dict = data_loader._get_local_data(
            symbols, "tests/testdata/local", required_columns=["Adj Close"]
        )

        assert len(data_dict) == 4
        assert len(error_dict) == 4

        assert set(error_dict.keys()) == set(
            ["TEST", "TEST_EMPTY", "TEST_NO_DATA", "TEST_NEXIST"]
        )

        message = (
            "The file for the symbol TEST only contains "
...
class TestGetAllData:
    def test_all_only_local(self):
        ...
        data_dict, error_local, error_yahoo = data_loader._get_all_data(
            ["ABBV", "ACGL"], data_path="tests/testdata/local", store_downloads=False
        )
    ...
    def test_all_only_yahoo(self):
        data_loader = data.RealDataLoader(download_delay=0)
        data_dict, error_local, error_yahoo = data_loader._get_all_data(
            ["AFL", "A"], data_path="tests/testdata/local", store_downloads=False
        )
    ...
    def test_all_mixed(self):
        data_loader = data.RealDataLoader(download_delay=0)
        data_dict, error_local, error_yahoo = data_loader._get_all_data(
            ["ABBV", "AIZ"], data_path="tests/testdata/local"
        )
...
class Test_Merge_Symbols:
    def merge_two_test(self):
        ...
        data_ = data.RealDataLoader()._merge_columns(
            column_name="Adj Close", data_dict=data_dict
        )
