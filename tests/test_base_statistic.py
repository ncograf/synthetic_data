...
class TestBaseStatistic:
    ...
    def test_check_data_validity(self):
        ...
        base_df = pd.DataFrame(
            data=base_stat.statistic, index=base_stat._dates, columns=base_stat._symbols
        )
        base_df_no_time = pd.DataFrame(
            data=base_stat.statistic, columns=base_stat._symbols
        )
        base_df_no_col = pd.DataFrame(
            data=base_stat.statistic, index=base_stat._dates, columns=[1, 2, 3, 4]
        )
