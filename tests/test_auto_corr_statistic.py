...
class TestAutoCorrStatistic:
    def test_get_statistic_simple_boosted(self):
        ...
        base_stat = stat.AutoCorrStatistic(
            max_lag=2, underlaying=log_stat, implementation="boosted"
        )
        ...
        assert np.array_equal(
            base_stat.statistic.round(6),
            np.array([[-1, 1], [-1, 1], [-1, 1]]).T,
            equal_nan=True,
        )
    ...
    def test_get_statistic_simple(self):
        log_stat = lstat.LogReturnStatistic(0.001)
        base_stat = stat.AutoCorrStatistic(
            max_lag=2, underlaying=log_stat, implementation="strides"
        )
    ...
    def test_get_statistic_python_loop(self):
        log_stat = lstat.LogReturnStatistic(0.001)
        base_stat = stat.AutoCorrStatistic(
            max_lag=4, underlaying=log_stat, implementation="python_loop"
        )
        _symbols = ["a", "b", "c", "d"]
    ...
    def test_get_statistic(self):
        log_stat = lstat.LogReturnStatistic(0.001)
        base_stat = stat.AutoCorrStatistic(
            max_lag=4, underlaying=log_stat, implementation="strides"
        )
        _symbols = ["a", "b", "c", "d"]
        _returns = np.array([2, 1, 2, 1, 2, 1, 2, 1, 2, 1])
        n_dates = _returns.shape[0] + 1
    ...
    def test_get_statistic_boosted(self):
        log_stat = lstat.LogReturnStatistic(0.001)
        base_stat = stat.AutoCorrStatistic(
            max_lag=4, underlaying=log_stat, implementation="boosted"
        )
        _symbols = ["a", "b", "c", "d"]
        _returns = np.array([2, 1, 2, 1, 2, 1, 2, 1, 2, 1])
        n_dates = _returns.shape[0] + 1
