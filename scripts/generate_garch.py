...
@click.option("-p", type=int, help="Parameter p for garch model")
@click.option("-q", type=int, help="Parameter q for garch model")
...
def generate_garch_index(dist: str, p, q, num_cpu):
    ...
    if dist.lower() == "studentt":
        ...
        raise ValueError(
            "The distribution must be chosen out of the predefined options."
        )
