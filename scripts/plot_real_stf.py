...
def main(png):
    """Create pgf vector graphics (or png images) for real data experiments as described in the thesis"""
    ...
    if (data_dir / "split_data.json").exists():
        ...
        for _ in range(R):
            ...
            stf_one = stylized_score.boostrap_stylized_facts(
                log_ret[:, idx_one], B, S, L
            )
            stf_two = stylized_score.boostrap_stylized_facts(
                log_ret[:, idx_two], B, S, L
            )
