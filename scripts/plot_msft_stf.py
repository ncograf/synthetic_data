...
@click.option(
    "-s", "--sym", type=str, default="msft", help="Symbols used for experiments"
)
def main(png, sym):
    """Create pgf vector graphics (or png images) for single stock experiments
    as described in the thesis

    The method assumes that the directory ${DATA_DIR}/{sym}_fingan_runs exists and contains
    experiemnts, as copied by the script `get_runs.sh`
    """
    ...
    if (data_dir / f"{sym}_data.json").exists():
        with (data_dir / f"{sym}_data.json").open() as file:
        ...
        msft_data = [(tot, sc, w, sym.upper())]
        with (data_dir / f"{sym}_data.json").open("w") as file:
    ...
    if pgf:
        ...
        plot_table.create_table(
            f"{sym}_{sym}_stf_data",
            f"{sym}_stf_data",
            "(c)",
            f"S\\&P 500 vs Real {sym.upper()} data",
