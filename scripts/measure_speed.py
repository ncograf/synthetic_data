...
def measure_abs_log_return():
    ...
    for i in lags:
        ...
        log_corr_stat = auto_corr_statistic.AutoCorrStatistic(
            max_lag=i, underlaying=log_stat, implementation="strides"
        )
    ...
    for i in lags:
        start = time.time()
        log_corr_stat = auto_corr_statistic.AutoCorrStatistic(
            max_lag=i, underlaying=log_stat, implementation="boosted"
        )
        log_corr_stat.set_statistics(None)
        stop = time.time()
    ...
    for i in lags:
        start = time.time()
        log_corr_stat = auto_corr_statistic.AutoCorrStatistic(
            max_lag=i, underlaying=log_stat, implementation="python_loop"
        )
        log_corr_stat.set_statistics(None)
        stop = time.time()
